...
static Atlas_Dim GenerateMeshAtlas(Scene& scene, Entity entity, uint32_t resolution)
{
	...
	// Generate atlas:
	{
		...
		lb::vector<float> softbodyweights;
		if (!meshcomponent.vertex_normals.empty())
		{
...
void ObjectWindow::Create(EditorComponent* _editor)
{
	...
	generateLightmapButton.OnClick([&](lb::gui::EventArgs args) {
		...
		lb::unordered_map<Entity, Atlas_Dim> gen_meshes;
	...
	vertexAOButton.OnClick([&](lb::gui::EventArgs args) {
		...
		if (!deleteAOMode)
		{
			...
			lb::console::Post("Building BVHs for vertex AO took " + lb::helper::GetTimerDurationText((float)timer.elapsed_seconds()));
		}
		...
		for (auto& x : this->editor->translator.selected)
		{
			...
			if (objectcomponent != nullptr)
			{
				...
				else
				{
					...
					lb::jobsystem::Wait(ctx);
					lb::console::Post("Vertex AO baking took " + lb::helper::GetTimerDurationText((float)timer.elapsed_seconds()));
				}
