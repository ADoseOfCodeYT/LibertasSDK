...
namespace lb
{
	...
	public:
		...
		void UpdateDisplacementMap(lb::graphics::CommandList cmd) const;
		void RenderForOcclusionTest(const lb::scene::CameraComponent& camera, lb::graphics::CommandList cmd) const;
		void Render(const lb::scene::CameraComponent& camera, lb::graphics::CommandList cmd) const;

		void CopyDisplacementMapReadback(lb::graphics::CommandList cmd) const;

		const lb::graphics::Texture* getDisplacementMap() const;
		const lb::graphics::Texture* getGradientMap() const;
		...
		mutable int occlusionQueries[lb::graphics::GraphicsDevice::GetBufferCount()];
	...
	protected:
		lb::graphics::Texture displacementMap;		// (RGBA32F)
		lb::graphics::Texture gradientMap;			// (RGBA16F)

		lb::graphics::Texture displacementMap_readback[lb::graphics::GraphicsDevice::GetBufferCount()];		// (RGBA32F)
		...
		lb::graphics::GPUBuffer buffer_Float2_H0;

		// Angular frequency
		lb::graphics::GPUBuffer buffer_Float_Omega;

		// Height field H(t), choppy field Dx(t) and Dy(t) in frequency domain, updated each frame.
		lb::graphics::GPUBuffer buffer_Float2_Ht;

		// Height & choppy buffer in the space domain, corresponding to H(t), Dx(t) and Dy(t)
		lb::graphics::GPUBuffer buffer_Float_Dxyz;


		lb::graphics::GPUBuffer constantBuffer;
		mutable lb::graphics::GPUBuffer indexBuffer;
		mutable lb::graphics::GPUBuffer indexBuffer_occlusionTest;
	};
}
