...
namespace lb::lua::primitive
{
	...
	Ray_BindLua::Ray_BindLua(lua_State *L)
	{
		...
		if (argc > 1)
		{
			...
			if (o && d)
			{
				...
				if (argc > 2)
				{
					ray.TMin = lb::lua::SGetFloat(L, 3);
				}
				if (argc > 3)
				{
					ray.TMax = lb::lua::SGetFloat(L, 4);
				}
			}
			else
			{
				lb::lua::SError(L, "Ray(Vector origin,direction, opt float Tmin = 0, TMax = FLT_MAX) requires two arguments of Vector type!");
			}
		}
		else
		{
			lb::lua::SError(L, "Ray(Vector origin,direction) not enough arguments!");
		}
	}
	...
	int Ray_BindLua::Intersects(lua_State* L)
	{
		...
		lb::lua::SError(L, "[Intersects(AABB), Intersects(Sphere), Intersects(Capsule), Intersects(Vector)] no matching arguments! ");
		return 0;
	}
	...
	int Ray_BindLua::SetOrigin(lua_State *L)
	{
		...
		if (argc > 0)
		{
			...
			{
				lb::lua::SError(L, "SetOrigin(Vector vector) argument is not a vector!");
			}
		}
		else
		{
			lb::lua::SError(L, "SetOrigin(Vector vector) not enough arguments!");
		}
		return 0;
	}
	...
	int Ray_BindLua::SetDirection(lua_State *L)
	{
		...
		if (argc > 0)
		{
			...
			{
				lb::lua::SError(L, "SetDirection(Vector vector) argument is not a vector!");
			}
		}
		else
		{
			lb::lua::SError(L, "SetDirection(Vector vector) not enough arguments!");
		}
		return 0;
	}
	int Ray_BindLua::CreateFromPoints(lua_State* L)
	{
		...
		if (argc > 1)
		{
			...
			{
				lb::lua::SError(L, "CreateFromPoints(Vector a,b) argument is not a vector!");
			}
		}
		else
		{
			lb::lua::SError(L, "CreateFromPoints(Vector a,b) not enough arguments!");
		}
		return 0;
	}
	int Ray_BindLua::GetPlacementOrientation(lua_State* L)
	{
		...
		if (argc > 1)
		{
			...
			if (position == nullptr)
			{
				lb::lua::SError(L, "GetPlacementOrientation(Vector position, normal) first argument is not a Vector!");
				return 0;
			}
			Vector_BindLua* normal = Luna<Vector_BindLua>::lightcheck(L, 2);
			if (normal == nullptr)
			{
				lb::lua::SError(L, "GetPlacementOrientation(Vector position, normal) second argument is not a Vector!");
				return 0;
			}
			Luna<Matrix_BindLua>::push(L, ray.GetPlacementOrientation(position->GetFloat3(), normal->GetFloat3()));
			return 1;
		}
		lb::lua::SError(L, "GetPlacementOrientation(Vector position, normal) not enough arguments!");
		return 0;
	}
	...
	AABB_BindLua::AABB_BindLua(lua_State *L)
	{
		...
		if (argc > 1)
		{
			...
			{
				lb::lua::SError(L, "AABB(opt Vector min,max) arguments must be of Vector type!");
			}
		}
		else
		{
	...
	int AABB_BindLua::Intersects(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (_vec)
			{
				lb::lua::SSetBool(L, aabb.intersects(_vec->GetFloat3()));
				return 1;
			}
			...
			if (_aabb)
			{
				...
				//lb::lua::SSetInt(L, intersects);
				lb::lua::SSetBool(L, aabb.intersects(_aabb->aabb) != AABB::INTERSECTION_TYPE::OUTSIDE); // int intersection type cannot be checked like bool in lua so we give simple bool result here!
				return 1;
			}
		...
		lb::lua::SError(L, "[Intersects(AABB), Intersects(Sphere), Intersects(Ray)] no matching arguments! ");
		return 0;
	}
	int AABB_BindLua::Intersects2D(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (_aabb)
			{
				//int intersects = (int)aabb.intersects(_aabb->aabb);
				//lb::lua::SSetInt(L, intersects);
				lb::lua::SSetBool(L, aabb.intersects2D(_aabb->aabb) != AABB::INTERSECTION_TYPE::OUTSIDE); // int intersection type cannot be checked like bool in lua so we give simple bool result here!
				return 1;
			}

		}
		lb::lua::SError(L, "Intersects2D(AABB) not enough arguments! ");
		return 0;
	}
	...
	int AABB_BindLua::SetMin(lua_State *L)
	{
		...
		if (argc > 0)
		{
			...
			{
				lb::lua::SError(L, "SetMin(Vector vector) argument is not a vector!");
			}
		}
		else
		{
			lb::lua::SError(L, "SetMin(Vector vector) not enough arguments!");
		}
		return 0;
	}
	...
	int AABB_BindLua::SetMax(lua_State *L)
	{
		...
		if (argc > 0)
		{
			...
			{
				lb::lua::SError(L, "SetMax(Vector vector) argument is not a vector!");
			}
		}
		else
		{
			lb::lua::SError(L, "SetMax(Vector vector) not enough arguments!");
		}
		return 0;
	}
	...
	int AABB_BindLua::Transform(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			{
				lb::lua::SError(L, "Transform(Matrix matrix) argument is not a Matrix! ");
			}
		}
		lb::lua::SError(L, "Transform(Matrix matrix) not enough arguments! ");
		return 0;
	}
	...
	Sphere_BindLua::Sphere_BindLua(lua_State *L)
	{
		...
		if (argc > 1)
		{
			...
			if (cV)
			{
				...
				float r = lb::lua::SGetFloat(L, 2);

				sphere = Sphere(c, r);
			}
			else
			{
				lb::lua::SError(L, "Sphere(Vector center, float radius) requires first argument to be of Vector type!");
			}
		}
		else
		{
			lb::lua::SError(L, "Sphere(Vector center, float radius) not enough arguments!");
		}
	}
	...
	{
		lb::lua::SSetFloat(L, sphere.radius);
		return 1;
	}
	int Sphere_BindLua::SetCenter(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			{
				lb::lua::SError(L, "SetCenter(Vector value) requires first argument to be of Vector type!");
			}
		}
		else
		{
			lb::lua::SError(L, "SetCenter(Vector value) not enough arguments!");
		}
		return 0;
	}
	int Sphere_BindLua::SetRadius(lua_State* L)
	{
		...
		if (argc > 0)
		{
			sphere.radius = lb::lua::SGetFloat(L, 1);
		}
		else
		{
			lb::lua::SError(L, "SetRadius(float value) not enough arguments!");
		}
		return 0;
	}
	...
	Capsule_BindLua::Capsule_BindLua(lua_State* L)
	{
		...
		if (argc > 2)
		{
			...
			if (bV && tV)
			{
				...
				float r = lb::lua::SGetFloat(L, 3);

				capsule = Capsule(b, t, r);
			}
			else
			{
				lb::lua::SError(L, "Capsule(Vector base, tip, float radius) requires first two arguments to be of Vector type!");
			}
		}
		else
		{
			lb::lua::SError(L, "Capsule(Vector base, tip, float radius) not enough arguments!");
		}
	}
	...
	int Capsule_BindLua::Intersects(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (ray)
			{
				lb::lua::SSetBool(L, capsule.intersects(ray->ray));
				return 1;
			}
			...
			if (point)
			{
				lb::lua::SSetBool(L, capsule.intersects(point->GetFloat3()));
				return 1;
			}
		}
		lb::lua::SError(L, "Intersects(Capsule/Ray/Vector other) no matching arguments! ");
		return 0;
	}
	...
	{
		lb::lua::SSetFloat(L, capsule.radius);
		return 1;
	}
	int Capsule_BindLua::SetBase(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			{
				lb::lua::SError(L, "SetBase(Vector value) requires first argument to be of Vector type!");
			}
		}
		else
		{
			lb::lua::SError(L, "SetBase(Vector value) not enough arguments!");
		}
		return 0;
	}
	int Capsule_BindLua::SetTip(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			{
				lb::lua::SError(L, "SetTip(Vector value) requires first argument to be of Vector type!");
			}
		}
		else
		{
			lb::lua::SError(L, "SetTip(Vector value) not enough arguments!");
		}
		return 0;
	}
	int Capsule_BindLua::SetRadius(lua_State* L)
	{
		...
		if (argc > 0)
		{
			capsule.radius = lb::lua::SGetFloat(L, 1);
		}
		else
		{
			lb::lua::SError(L, "SetRadius(float value) not enough arguments!");
