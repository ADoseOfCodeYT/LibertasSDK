...
namespace lb
{
	struct Timer
	{
		...
		// Elapsed time in seconds between the lb::Timer creation or last recording and "timestamp2"
		...
		// Elapsed time in seconds since the lb::Timer creation or last recording
		inline double elapsed_seconds()
		{
			return elapsed_seconds_since(std::chrono::high_resolution_clock::now());
		}

		// Elapsed time in milliseconds since the lb::Timer creation or last recording
		...
		// Record a reference timestamp and return elapsed time in seconds since the lb::Timer creation or last recording
