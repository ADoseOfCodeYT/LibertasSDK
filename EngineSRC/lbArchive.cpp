...
namespace lb
{
		...
		: readMode(readMode), fileName(fileName)
	{
		if (!fileName.empty())
		{
			directory = lb::helper::GetDirectoryFromPath(fileName);
			if (readMode)
			{
				if (lb::helper::FileRead(fileName, DATA))
				{
					data_ptr = DATA.data();
	...
	{
		return lb::helper::FileWrite(fileName, data_ptr, pos);
	}

	bool Archive::SaveHeaderFile(const std::string& fileName, const std::string& dataName)
	{
		return lb::helper::Bin2H(data_ptr, pos, fileName, dataName.c_str());
	}
	...
	lb::graphics::Texture Archive::CreateThumbnailTexture() const
	{
		...
		lb::texturehelper::CreateTexture(texture, rgba, (uint32_t)width, (uint32_t)height);
		stbi_image_free(rgba);
		return texture;
	}

	void Archive::SetThumbnailAndResetPos(const lb::graphics::Texture& texture)
	{
		lb::vector<uint8_t> thumbnail_data;
		lb::helper::saveTextureToMemoryFile(texture, "JPG", thumbnail_data);
	...
	lb::graphics::Texture Archive::PeekThumbnail(const std::string& filename)
	{
		lb::vector<uint8_t> filedata;

		size_t required_size = sizeof(uint64_t) * 2; // version and thumbnail data size

		lb::helper::FileRead(filename, filedata, required_size); // read only up to version and thumbnail data size
		if (filedata.empty())
			return {};

		{
			lb::Archive archive(filedata.data(), filedata.size());
			if (archive.IsOpen())
			{
		...
		lb::helper::FileRead(filename, filedata, required_size); // read up to end of thumbnail data
		if (filedata.empty())
			return {};
