...
using namespace vulkan_internal;
	...
	GraphicsDevice_Vulkan::GraphicsDevice_Vulkan(lb::platform::window_type window, ValidationMode validationMode_, GPUPreference preference)
	{
		...
		if (res != VK_SUCCESS)
		{
			lb::helper::messageBox("volkInitialize failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
			lb::platform::Exit();
		}
		...
		// Create instance:
		{
			...
			if (res != VK_SUCCESS)
			{
				lb::helper::messageBox("vkCreateInstance failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
				lb::platform::Exit();
			}
		...
		// Enumerating and creating devices:
		{
			...
			{
				assert(0);
				lblog_messagebox("Failed to find GPU with Vulkan 1.3 support!");
				lb::platform::Exit();
			}
			...
			{
				lblog_messagebox("Failed to find a suitable GPU!");
				lb::platform::Exit();
			}
			...
			if (res != VK_SUCCESS)
			{
				lb::helper::messageBox("vkCreateDevice failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
				lb::platform::Exit();
			}
		...
		if (res != VK_SUCCESS)
		{
			lb::helper::messageBox("vmaCreateAllocator failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
			lb::platform::Exit();
		}
		...
		if (res != VK_SUCCESS)
		{
			lb::helper::messageBox("Failed to create Vulkan external memory allocator, ERROR: " + std::string(string_VkResult(res)), "Error!");
			lb::platform::Exit();
		}
		...
		for (uint32_t fr = 0; fr < BUFFERCOUNT; ++fr)
		{
			for (int queue = 0; queue < QUEUE_COUNT; ++queue)
			{
				...
				if (res != VK_SUCCESS)
				{
					lb::helper::messageBox("vkCreateFence[FRAME] failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
					lb::platform::Exit();
				}
			}
		}
...
#else
#error LIBERTASENGINE VULKAN DEVICE ERROR: PLATFORM NOT SUPPORTED
#endif // _WIN32
		}
