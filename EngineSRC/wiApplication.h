...
namespace lb
{
	...
	{
	protected:
		std::unique_ptr<lb::graphics::GraphicsDevice> graphicsDevice;
		lb::eventhandler::Handle swapChainVsyncChangeEvent;
		...
		lb::graphics::Texture rendertarget;
	...
	public:
		...
		bool allow_hdr = true;
		lb::graphics::SwapChain swapChain;
		lb::Canvas canvas;
		lb::platform::window_type window;
		...
		void ActivatePath(RenderPath* component, float fadeSeconds = 0, lb::Color fadeColor = lb::Color(0, 0, 0, 255));
		...
		virtual void Compose(lb::graphics::CommandList cmd);

		// You need to call this before calling Run() or Initialize() if you want to render
		void SetWindow(lb::platform::window_type);
