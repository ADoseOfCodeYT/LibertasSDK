	...
	struct NetworkInternal
	{
		...
		NetworkInternal()
		{
			...
			if (result)
			{
				...
				lb::backlog::post("lb::network Initialization FAILED with error: " + std::to_string(error));
				assert(0);
			}

			lb::backlog::post("lb::network Initialized (" + std::to_string((int)std::round(timer.elapsed())) + " ms)");
		}
		~NetworkInternal()
		{
			WSACleanup();
