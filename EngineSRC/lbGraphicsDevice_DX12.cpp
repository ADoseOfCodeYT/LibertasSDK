...
namespace dx12_internal
{
	...
	struct RootSignatureOptimizer
	{
		...
		lb::vector<RootParameterStatistics> root_stats;
	...
	struct Resource_DX12
	{
		...
		lb::vector<SingleDescriptor> subresources_srv;
		lb::vector<SingleDescriptor> subresources_uav;
		SingleDescriptor uav_raw;

		D3D12_GPU_VIRTUAL_ADDRESS gpu_address = 0;

		UINT64 total_size = 0;
		lb::vector<D3D12_PLACED_SUBRESOURCE_FOOTPRINT> footprints;
		lb::vector<UINT64> rowSizesInBytes;
		lb::vector<UINT> numRows;
	...
	struct Texture_DX12 : public Resource_DX12
	{
		...
		lb::vector<SingleDescriptor> subresources_rtv;
		lb::vector<SingleDescriptor> subresources_dsv;
	...
	struct PipelineState_DX12
	{
		...
		lb::vector<uint8_t> shadercode;
		lb::vector<D3D12_INPUT_ELEMENT_DESC> input_elements;
	...
	struct BVH_DX12 : public Resource_DX12
	{
		...
		lb::vector<D3D12_RAYTRACING_GEOMETRY_DESC> geometries;
	...
	struct RTPipelineState_DX12
	{
		...
		lb::vector<std::wstring> export_strings;
		lb::vector<D3D12_EXPORT_DESC> exports;
		lb::vector<D3D12_DXIL_LIBRARY_DESC> library_descs;
		lb::vector<std::wstring> group_strings;
		lb::vector<D3D12_HIT_GROUP_DESC> hitgroup_descs;
...
#endif // PLATFORM_XBOX
		lb::vector<ComPtr<ID3D12Resource>> backBuffers;
		lb::vector<D3D12_CPU_DESCRIPTOR_HANDLE> backbufferRTV;
...
std::mutex queue_locker;
	...
	void GraphicsDevice_DX12::CopyAllocator::init(GraphicsDevice_DX12* device)
	{
		...
		if (FAILED(hr))
		{
			lblog_messagebox("ID3D12Device::CreateCommandQueue[CopyAllocator] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
			lb::platform::Exit();
		}
	...
	GraphicsDevice_DX12::CopyAllocator::CopyCMD GraphicsDevice_DX12::CopyAllocator::allocate(uint64_t staging_size)
	{
		...
		if (!cmd.IsValid())
		{
			...
			uploadBufferDesc.size = lb::math::GetNextPowerOfTwo(staging_size);
	...
	GraphicsDevice_DX12::GraphicsDevice_DX12(ValidationMode validationMode_, GPUPreference preference)
	{
		...
		if (dxgi == nullptr)
		{
			...
			lb::helper::messageBox(ss.str(), "Error!");
			lb::platform::Exit();
		}
		...
		hr = lb::graphics::xbox::CreateDevice(device, dxgiAdapter, validationMode);

#else
		hr = CreateDXGIFactory2((validationMode != ValidationMode::Disabled) ? DXGI_CREATE_FACTORY_DEBUG : 0u, PPV_ARGS(dxgiFactory));
		if (FAILED(hr))
		{
			lblog_messagebox("CreateDXGIFactory2 failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
			lb::platform::Exit();
		}
		...
		// Determines whether tearing support is available for fullscreen borderless windows.
		{
			...
			if (FAILED(hr) || !allowTearing)
			{
				...
				lb::helper::DebugOut("WARNING: Variable refresh rate displays not supported\n");
			}
			else
			{
		...
		{
			lblog_messagebox("DXGI: No capable graphics adapter found!");
			lb::platform::Exit();
		}
		...
		if (device == nullptr)
		{
			lblog_messagebox("D3D12: Device couldn't be created!");
			lb::platform::Exit();
		}
		...
		if (FAILED(hr))
		{
			lblog_messagebox("D3D12MA::CreateAllocator failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
			lb::platform::Exit();
		}

		{
			...
			if (FAILED(hr))
			{
				lblog_messagebox("ID3D12Device::CreateCommandQueue[QUEUE_GRAPHICS] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
				lb::platform::Exit();
			}
...

		{
			...
			if (FAILED(hr))
			{
				lblog_messagebox("ID3D12Device::CreateCommandQueue[QUEUE_COMPUTE] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
				lb::platform::Exit();
			}
...

		{
			...
			if (FAILED(hr))
			{
				lblog_messagebox("ID3D12Device::CreateCommandQueue[QUEUE_COPY] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
				lb::platform::Exit();
			}
		...
		// Resource descriptor heap (shader visible):
		{
			...
			if (FAILED(hr))
			{
				lblog_messagebox("ID3D12Device::CreateDescriptorHeap[CBV_SRV_UAV] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
				lb::platform::Exit();
			}
			...
			if (FAILED(hr))
			{
				lblog_messagebox("ID3D12Device::CreateFence[CBV_SRV_UAV] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
				lb::platform::Exit();
			}
		...
		// Sampler descriptor heap (shader visible):
		{
			...
			if (FAILED(hr))
			{
				lblog_messagebox("ID3D12Device::CreateDescriptorHeap[SAMPLER] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
				lb::platform::Exit();
			}
			...
			if (FAILED(hr))
			{
				lblog_messagebox("ID3D12Device::CreateFence[SAMPLER] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
				lb::platform::Exit();
			}
		...
		for (uint32_t buffer = 0; buffer < BUFFERCOUNT; ++buffer)
		{
			for (int queue = 0; queue < QUEUE_COUNT; ++queue)
			{
				...
				if (FAILED(hr))
				{
					lblog_messagebox("ID3D12Device::CreateFence[FRAME] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
					lb::platform::Exit();
				}
			}
		}
...
#ifdef PLATFORM_XBOX
		adapterName = lb::graphics::xbox::GetAdapterName();
		...
		// Init adapter properties
		{
			...
			lb::helper::StringConvert(adapterDesc.Description, adapterName);
		...
		if (features.HighestShaderModel() < D3D_SHADER_MODEL_6_0)
		{
			...
			switch (adapterType)
			{
			case lb::graphics::AdapterType::IntegratedGpu:
				error += "Integrated GPU";
				break;
			case lb::graphics::AdapterType::DiscreteGpu:
				error += "Discrete GPU";
				break;
			case lb::graphics::AdapterType::VirtualGpu:
				error += "Virtual GPU";
				break;
			case lb::graphics::AdapterType::Cpu:
				error += "CPU";
				break;
			default:
				error += "Unknown"; // i will be surprised if anyone manages to get this error
				break;
			}
			error += "\nExiting.";
			lblog_messagebox(error.c_str());
			lb::platform::Exit();
		}
		...
		{
			lblog_messagebox("DX12: Root signature version 1.1 not supported!");
			lb::platform::Exit();
		}
		...
		if (FAILED(hr))
		{
			lblog_messagebox("ID3D12Device::CreateCommandSignature[dispatchIndirect] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
			lb::platform::Exit();
		}
		...
		if (FAILED(hr))
		{
			lblog_messagebox("ID3D12Device::CreateCommandSignature[drawInstancedIndirect] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
			lb::platform::Exit();
		}
		...
		if (FAILED(hr))
		{
			lblog_messagebox("ID3D12Device::CreateCommandSignature[drawIndexedInstancedIndirect] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
			lb::platform::Exit();
		}
		...
		if (CheckCapability(GraphicsDeviceCapability::MESH_SHADER))
		{
...
#ifdef PLATFORM_XBOX
			lb::graphics::xbox::FillDispatchMeshIndirectArgumentDesc(dispatchMeshArgs[0], cmd_desc);
#else
			...
			if (FAILED(hr))
			{
				lblog_messagebox("ID3D12Device::CreateCommandSignature[dispatchMeshIndirect] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
				lb::platform::Exit();
			}
		}
		...
		if (FAILED(hr))
		{
			lblog_messagebox("ID3D12Device::CreateDescriptorHeap[nulldescriptorheap_cbv_srv_uav] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
		}
		...
		if (FAILED(hr))
		{
			lblog_messagebox("ID3D12Device::CreateDescriptorHeap[nulldescriptorheap_sampler] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
		}
		...
		if (FAILED(hr))
		{
			lblog_messagebox("ID3D12CommandQueue::GetTimestampFrequency[QUEUE_GRAPHICS] failed! ERROR: %s", lb::helper::GetPlatformErrorString(hr).c_str());
		}

		lblog("Created Graphics Device [DX12] (%d ms)\nAdapter: %s", (int)std::round(timer.elapsed()), adapterName.c_str());
	}
	...
	bool GraphicsDevice_DX12::CreateSwapChain(const SwapChainDesc* desc, lb::platform::window_type window, SwapChain* swapchain) const
	{
...
#ifdef PLATFORM_XBOX
		lb::graphics::xbox::ApplyBufferCreationFlags(*desc, resourceDesc.Flags, allocationDesc.ExtraHeapFlags);
#endif // PLATFORM_XBOX
...
#ifdef PLATFORM_XBOX
		lb::graphics::xbox::ApplyTextureCreationFlags(texture->desc, resourcedesc.Flags, allocationDesc.ExtraHeapFlags);
#endif // PLATFORM_XBOX
	...
	bool GraphicsDevice_DX12::CreatePipelineState(const PipelineStateDesc* desc, PipelineState* pso, const RenderPassInfo* renderpass_info) const
	{
		...
		{
			lb::helper::hash_combine(internal_state->hash, renderpass_info->get_hash());
	...
	bool GraphicsDevice_DX12::CreateRaytracingPipelineState(const RaytracingPipelineStateDesc* desc, RaytracingPipelineState* rtpso) const
	{
		...
		lb::vector<D3D12_STATE_SUBOBJECT> subobjects;
		...
		for(auto& x : desc->shader_libraries)
		{
			...
			lb::helper::StringConvert(x.function_name, internal_state->export_strings.emplace_back());
		...
		{
			lb::helper::StringConvert(x.name, internal_state->group_strings.emplace_back());
	...
	bool GraphicsDevice_DX12::CreateVideoDecoder(const VideoDesc* desc, VideoDecoder* video_decoder) const
	{
		...
		lb::vector<GUID> profiles(video_decode_profile_count.ProfileCount);
		...
		lb::vector<DXGI_FORMAT> formats(video_decode_format_count.FormatCount);
	...
	{
		wchar_t text[256];
		if (lb::helper::StringConvert(name, text, arraysize(text)) > 0)
		{
	...
	void GraphicsDevice_DX12::OnDeviceRemoved()
	{
		...
		if (SUCCEEDED(device->QueryInterface(PPV_ARGS(pDred))))
		{
			...
			if (SUCCEEDED(pDred->GetAutoBreadcrumbsOutput1(&pDredAutoBreadcrumbsOutput)))
			{
				...
				while (pNode && pNode->pLastBreadcrumbValue)
				{
					...
					{
						lb::helper::StringConvert(pNode->pCommandListDebugNameW, commandlistname);
					}
					if (pNode->pCommandQueueDebugNameW != nullptr)
					{
						lb::helper::StringConvert(pNode->pCommandQueueDebugNameW, commandqueuename);
					}
					...
					for (int op = firstOp; op <= lastOp; ++op)
					{
						...
						{
							lb::helper::StringConvert(it->second, contextString);
						}
			...
			if (SUCCEEDED(pDred->GetPageFaultAllocationOutput1(&DredPageFaultOutput)) && DredPageFaultOutput.PageFaultVA != 0)
			{
				...
				if (pNode)
				{
					...
					while (pNode)
					{
						...
						{
							lb::helper::StringConvert(pNode->ObjectNameW, objectname);
						}
		...
		lb::helper::messageBox(message, "Error!");
		lb::platform::Exit();
#endif // PLATFORM_WINDOWS_DESKTOP
	}
	...
	void GraphicsDevice_DX12::SparseUpdate(QUEUE_TYPE queue, const SparseUpdateCommand* commands, uint32_t command_count)
	{
		...
		thread_local lb::vector<D3D12_TILED_RESOURCE_COORDINATE> tiled_resource_coordinates;
		thread_local lb::vector<D3D12_TILE_REGION_SIZE> tiled_region_sizes;
		thread_local lb::vector<D3D12_TILE_RANGE_FLAGS> tile_range_flags;
		thread_local lb::vector<uint32_t> range_start_offsets;
	...
	void GraphicsDevice_DX12::EventBegin(const char* name, CommandList cmd)
	{
		...
		if (commandlist.queue == QUEUE_VIDEO_DECODE)
			return;
		wchar_t text[128];
		if (lb::helper::StringConvert(name, text) > 0)
		{
